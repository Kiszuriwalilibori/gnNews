...
export const createSortedCountryCodesWithNames = (countries: string[]) => {
    if (!countries || isEmpty(countries)) {
        ...
        const ary = [...countries];
        const countriesWithNames = ary.map((countryCode: string) => {
            return { code: countryCode, name: getCountryName(countryCode) };
        });
        const sortedCountriesWithNames = countriesWithNames.sort((a, b) =>
            a.name > b.name ? 1 : a.name < b.name ? -1 : 0
        );
        return sortedCountriesWithNames;
    }
};
...
export const getSortedCountryCodesWithNames = createSelector(getCountriesList, createSortedCountryCodesWithNames);
